...
async def send_message_with_data(ctx, request, args, callback):
    ...
    if data is None:
        ...
        if request['type'] == 'movie' or request['type'] == 'TV show':
            imdb_button = Button(label="IMDB", style=discord.ButtonStyle.link,
                                 url=f"https://www.imdb.com/title/{data['tconst']}")
...
def get_online_channel_members(ctx):
    return [f"{m.name}#{m.discriminator}" for m in ctx.channel.members if
            not m.bot and m.status is discord.Status.online]
